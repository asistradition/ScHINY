...
plot.figure.2.ridgeline <- function(shiny.data, gene.vec, input) {
  ...
  plot.stack <- plot_grid(plt.labels, plot_grid(plotlist = plt.list, ncol = length(plt.list), align="h"), ncol=2, rel_widths = c(.1, 1), align="hv")
...
make.ridgeline.plt <- function(ridge.data, median.data, gene.name) {
  ...
  ggplot(ridge.data, aes(x=Count, y=Condition)) +
    stat_binline(aes(height=..ndensity.., fill=factor(Condition)), binwidth=1, center=0, draw_baseline = FALSE, scale = 0.9) + 
    geom_point(data=median.data[median.data$Gene == gene.name, ], aes(x=gene_median, y=Condition)) +
